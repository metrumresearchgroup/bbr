...
NONMEM_ARGS = list(
  cache_dir = list(type = "character", flag="--cacheDir", description = "directory path for cache of nonmem executables for NM7.4+"),
  cache_exe = list(type = "character", flag="--cacheExe", description = "name of executable stored in cache"),
  clean_lvl = list(type = "numeric", flag="--cleanLvl", description = "clean level used for file output from a given (set of) runs (default 1)"),
  config = list(type = "character", flag="--config", description = "config file (default is $HOME/babylon.yaml)"),
  copy_lvl = list(type = "numeric", flag="--copyLvl", description = "copy level used for file output from a given (set of) runs"),
  ...
  mpi_exec_path = list(type = "character", flag="--mpiExecPath", description = "The fully qualified path to mpiexec. Used for nonmem parallel operations (default '/usr/local/mpich3/bin/mpiexec')"),
  nm_version = list(type = "character", flag="--nmVersion", description = "Version of nonmem from the configuration list to use"),
  ...
  output_dir = list(type = "character", flag="--outputDir", description = "Go template for the output directory to use for storging details of each executed model (default '{{ .Name}}')"),
  ...
  saveConfig = list(type = "logical", flag="--saveConfig", description = "Whether or not to save the existing configuration to a file with the model (default true)"),
  save_exe = list(type = "character", flag="--saveExe", description = "what to name the executable when stored in cache"),
