...
#' Check that a Stan model is up-to-date
#'
#' Specifically, check that required files on disk
#' have not changed since the model was run. This is accomplished
#' by taking their md5 hashes and comparing it to the hashes
...
#'
#' @param .mod a `bbi_stan_model` object
#' @param .build_data If `FALSE`, the default, check the hashes of the
#'   `-standata.R` and `-standata.json` against the hashes in `bbi_config.json`.
#'   If `TRUE`, run `-standata.R` and save the output to a temp file and check
#'   the hash of the temp file against the `bbi_config.json` hash. The second
#'   option actually runs the code and, importantly, as verifies that the input
#'   data has not changed.
#'
#' @return `TRUE` if nothing has changed, `FALSE` if anything has changed.
#' @keywords internal
check_stan_output_up_to_date <- function(.mod, .build_data = FALSE) {

  # check model and load config
  check_stan_model(.mod, error = TRUE)
  config_path <- file.path(get_output_dir(.mod), "bbi_config.json")
  if (!fs::file_exists(config_path)) {
    stop(glue("Cannot check if {get_model_id(.mod)} is up-to-date because it has not been run."))
  }
  ...
  changed_files <- c(
    changed_files,
    config[[CONFIG_MODEL_MD5]] != tools::md5sum(get_model_path(.mod))
  )

  changed_files <- c(
    changed_files,
    config[[STANCFG_INIT_MD5]] != tools::md5sum(build_path_from_model(.mod, STANINIT_SUFFIX))
  )

  changed_files <- c(
    changed_files,
    config[[STANCFG_ARGS_MD5]] != tools::md5sum(build_path_from_model(.mod, STANARGS_SUFFIX))
  )
  ...
  if (isTRUE(.build_data)) {
    ...
    changed_files <- c(
      changed_files,
      config[[STANCFG_DATA_MD5]] != tools::md5sum(build_path_from_model(.mod, STANDATA_R_SUFFIX)),
      config[[CONFIG_DATA_MD5]] != tools::md5sum(build_path_from_model(.mod, STANDATA_JSON_SUFFIX))
    )
  }
