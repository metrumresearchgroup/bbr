...
Now that you have arrived at your final model, you can add description to identify it, which will be used shortly for filtering the `run_log()` tibble.
...
Among other things, the run log contains any descriptions that have been assigned to each model. Here we use `filter` and `pull` to get the path to the final model.
