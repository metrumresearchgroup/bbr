...
This will install to the `/data/apps/` folder by default, which will usually match the `BBI_EXE_PATH` you just defined (it will not match only if the `BABYLON_EXE_PATH` environment variable is deliberately set to a different path). If you have changed that, you can pass your custom path to the _directory_ where you want to put bbi like so `use_bbi("/some/other/dir")`. In that case, you would want to set `BBI_EXE_PATH = "/some/other/dir/bbi"`.
...
### Overwriting output from previously a run model
