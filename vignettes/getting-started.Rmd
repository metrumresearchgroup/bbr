...
The `rbabylon::collapse()` function can be passed any column that is a list of vectors. It is specifically designed for collapsing columns like `tags`, `notes`, and `based_on` to a more human-readable format.

```{r viewing tags new} 
log_df %>% 
  rbabylon::collapse(tags, notes) %>%
