...
*NOTE:* By setting the `rbabylon.model_directory` option, you will _not_ need to pass the full path to model files, but only the path relative to the specified modeling directory. This is not necessary, but it is definitely a recommended convenience. By contrast, setting `rbabylon.bbi_exe_path` _is necessary_ for `rbabylon` to be able to find `bbi` on the system.
