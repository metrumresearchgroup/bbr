...
Once `babylon` is installed and initialized, you will need to make `rbabylon` knows where to find it. This can be done by setting the option

```{r bbi_exe_path}
options('rbabylon.bbi_exe_path' = BBI_EXE_PATH)
```

This must be set for every new R session, so it may be wise to **include the above snippet in your `.Rprofile`**, replacing the `BBI_EXE_PATH` and `MODEL_DIR` constants with the relevant paths from your system/project.
