...
mod3 <- mod3 %>% 
        ...
        add_notes("Added residual errors because it seemed like a good idea.")
```
...
The `bbi_log_df` method of the `toString()` generic can be passed a tibble output from `run_log()`, `summary_log()`, or `config_log()`, along with any columns that are a list of vectors. It is specifically designed for collapsing columns like `tags`, `notes`, and `based_on` to a more human-readable format.
