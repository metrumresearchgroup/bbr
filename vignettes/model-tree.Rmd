...
copy_output_dir <- function(orig_mod, new_mod) {
  new_dir_path <- new_mod[[ABS_MOD_PATH]]
  fs::dir_copy(orig_mod[[ABS_MOD_PATH]], new_dir_path)
  ...
  purrr::walk(fs::dir_ls(new_dir_path), ~ {
    if (stringr::str_detect(basename(.x), glue("^{orig_mod_id}"))) {
      mod_name <- stringr::str_replace(basename(.x), glue("^{orig_mod_id}"), new_mod_id)
...
create_tree_models <- function(MODEL_DIR){
  ...
  mod4 <- copy_model_from(mod2, "4", .overwrite = TRUE)
  mod5 <- copy_model_from(mod4, "5", .overwrite = TRUE)
...
```{r, echo=FALSE}
# Since using fake models (all inheriting the same issues), set most to FALSE
log_df <- run_log(MODEL_DIR) %>% add_summary()
log_df$any_heuristics[1:(nrow(log_df) - 2)] <- FALSE
model_tree(
  log_df,
