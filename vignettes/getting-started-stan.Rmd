...
```{r}
new_res <- read_fit_model(file.path(MODEL_DIR, "fxaInc"))
new_res$sampler_diagnostics() %>% names()
```


### `model_summary()` function

We have **not yet implemented** `bbr::model_summary()` for `bbi_stan_model` objects. This is a work in progress, but until it is done, `model_summary()` currently returns the same fit object that was just discussed.

```{r model_summary}
sum1 <- fxaInc %>% model_summary()
class(sum1)
```

```{r}
sum1$sampler_diagnostics() %>% names()
```
