...
Note that if you already have the relevant code for your new model, you can manually create these files in the output directory _before_ calling `new_model()`. In that case, you won't see any of the messages shown above and the `new_model()` call will simply create the `.yaml` file next to your directory.
...
**WORK IN PROGRESS...** The submit a model to be run, you simply pass the model object to `bbr::submit_model()`. For Stan models, this wraps the `cmdstanr::sample` method. Any arguments that you would normally pass through to `$sample()` can be passed through `submit_model()` via `...`.
...
#### THIS IS TO RUN THE ORIGINAL MODEL
# It will be deleted from the final vingette because it will already be run
fxaORIG_res <- fxa %>%
  submit_model(
    seed = 123456,
    chains = nChains,
...
Importantly, **you must reassign to the model object** when calling any of these functions that modify it (notice the `fxaInc <- fxaInc ...` above). You can see a list of similar functions in the "See Also" section of [?modify_tags] or ?modify_notes].
