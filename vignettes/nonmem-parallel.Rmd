...
## Passing arguments to `submit_model()`
...
Once run, you can use `check_run_times()` to inspect the run times for each of the thread options you are benchmarking. If you specify `.wait = TRUE`, your R console will be frozen until the model run has finished, regardless of whether you submitted them to the grid or not.
