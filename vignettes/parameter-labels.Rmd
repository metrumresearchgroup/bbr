...
This vignette demonstrates parsing parameter estimates and labels into a table that can be used for diagnostics, or to generate reports. *Note, this vignette exclusively deals with NONMEM models.*
...
mod1 %>% 
  ...
  readr::read_lines(skip = 15, n_max = 18) %>%  # read in only parameter section
  paste(collapse = "\n") %>%
  cat()
```

This control stream is parsed into the tibble below, **following the syntax guidelines [here](https://ghe.metrumrg.com/pages/software/mrgtable/reference/nm_tbl.html) NEED EXTERNAL REFERENCE.**
...
```{r param_estimates}
report_df <- dplyr::inner_join(
  label_df %>% dplyr::select(-param_type),
  param_df %>% dplyr::select(names, estimate, stderr),
  by = "names"
)
report_df
```
...
There are two arguments, `.omega` and `.sigma` which each take a logical vector. Each element in this vector corresponds to a parameter specified in the control stream and denotes (with a `TRUE` or `FALSE`) whether that element is a diagonal in the relevant matrix.

For example, an `$OMEGA BLOCK(3)` would be represented with the vector `.omega = c(TRUE, FALSE, TRUE, FALSE, FALSE, TRUE)` because the first, third, and sixth elements are the diagonals and the others represent the off-diagonals. However, `rbabylon` has a helper function `block()` that generates these vectors.
...
Note this it is equally valid to replace any of these with logical vectors as well. For instance, instead of `c(..., block(1), block(1))` is may be easier to write `c(..., rep(TRUE, 2))`, especially if there are a number of `BLOCK(1)` lines in a row in the control stream.

## SAME blocks
It is also worth noting here that blocks like `$OMEGA BLOCK(1) SAME` which use `SAME` will inherit the label from the block to which `SAME` refers. This is true even if there are several same blocks in a row.
